...
public:	
	...
	void UpisiStudenta(std::string ime, std::string prezime, int indeks){
		if(!jel(indeks)) throw std::logic_error("Student sa zadanim brojem indeksa vec postoji");
	...
	void UpisiStudenta(std::string ime, std::string prezime, int indeks, int godina){
 	...
 	void ObrisiStudenta(int indeks){
 		if(jel(indeks)) throw std::logic_error("Student sa zadanim brojem indeksa ne postoji");
 		int poz=0;
 		for(int i=0; i < vektor.size(); i++){
